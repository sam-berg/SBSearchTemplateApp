...
function(declare, lang, array, html, topic, aspect, Deferred, query, nld, nlm, on, mouse, domAttr,
  domConstruct, domGeometry, registry, BaseWidget, PoolControllerMixin, tokenUtils, portalUrlUtils,
  utils, PopupTileNodes) {
  /* global jimuConfig */
  ...
  var clazz = declare([BaseWidget, PoolControllerMixin], {
    ...
    _highLight: function(node){
      ...
      this.hlDiv = domConstruct.create('div', {style:hlStyle}, node, 'before');
    },
    ...
    _createPopupLinkNode: function(){
      ...
      this.popupSigninNode = this._createLinkNode(node, {label: 'SignIn', url: '#'}, true);
      this.popupUserNameNode = this._createLinkNode(node, {label: '', url: '#'}, true);
      this.popupSignoutNode = this._createLinkNode(node, {label: 'SignOut', url: '#'}, true);

      on(this.popupSigninNode, 'click', (this._onSigninClick).bind(this));
      on(this.popupSignoutNode, 'click', (this._onSignoutClick).bind(this));
      //empty
      this._createLinkNode(node, {label: '', url: '#'}, false);
      return node;
    },
